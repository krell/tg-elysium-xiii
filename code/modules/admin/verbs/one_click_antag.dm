...
/datum/admins/proc/makeYellows()

	var/datum/game_mode/yellow/temp = new
	if(CONFIG_GET(flag/protect_roles_from_antagonist))
		temp.restricted_jobs += temp.protected_jobs
	...
	for(var/mob/living/carbon/human/applicant in GLOB.player_list)
		if(isReadytoRumble(applicant, ROLE_YELLOW))
			if(temp.age_check(applicant.client))
				if(!(applicant.job in temp.restricted_jobs))
					candidates += applicant

	if(candidates.len)
		var/numYellows = min(candidates.len, 3)

		for(var/i = 0, i<numYellows, i++)
			H = pick(candidates)
			H.mind.make_Yellow()
			candidates.Remove(H)
		return 1
